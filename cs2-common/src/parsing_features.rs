...
impl ParsingPreset {
    pub fn to_features(self) -> ParsingFeatures {
        match self {
            ParsingPreset::Minimal => ParsingFeatures::AIM | ParsingFeatures::UTILITY | ParsingFeatures::OBJECTIVE,
            ParsingPreset::Standard => ParsingFeatures::AIM
                | ParsingFeatures::MOVEMENT
                | ParsingFeatures::UTILITY
                | ParsingFeatures::OBJECTIVE
                | ParsingFeatures::INFO
                | ParsingFeatures::RULES,
            ParsingPreset::Rich => ParsingFeatures::AIM
                | ParsingFeatures::MOVEMENT
                | ParsingFeatures::UTILITY
                | ParsingFeatures::OBJECTIVE
...
pub fn build_wanted(features: ParsingFeatures) -> Wanted {
    ...
    if features.contains(ParsingFeatures::AIM) {
        player_props.extend([
            ...
            "m_angEyeAngles",
            "m_aimPunchAngle","m_aimPunchAngleVel","m_aimPunchTickBase","m_aimPunchTickFraction",
            "m_iHealth","m_ArmorValue","m_lifeState","m_bIsScoped",
            "m_hActiveWeapon","m_iClip1",
            "CCSPlayerPawn.CCSPlayer_WeaponServices.m_iAmmo",
            "m_fAccuracyPenalty","m_flRecoilIndex","m_fLastShotTime","m_bInReload",
            "m_flNextPrimaryAttackTickRatio","m_flNextSecondaryAttackTickRatio",
            "m_weaponMode","m_bBurstMode",
            "CCSPlayerPawn.m_iShotsFired","CCSPlayerPawn.CCSPlayer_BulletServices.m_totalHitsOnServer",
        ].into_iter().map(String::from));
        events.extend([
            "weapon_fire","player_hurt","player_death","weapon_reload","weapon_zoom"
        ].into_iter().map(String::from));
    }

    if features.contains(ParsingFeatures::MOVEMENT) {
        player_props.extend([
            "m_bIsWalking","is_airborne","m_fFlags","m_iMoveState","m_MoveType",
    ...
    if features.contains(ParsingFeatures::UTILITY) {
        events.extend([
            "flashbang_detonate","player_blind",
    ...
    if features.contains(ParsingFeatures::OBJECTIVE) {
        events.extend([
            "bomb_beginplant","bomb_planted","bomb_exploded",
            "bomb_begindefuse","bomb_defused","bomb_dropped","bomb_pickup",
        ].into_iter().map(String::from));
    }
    ...
    if features.contains(ParsingFeatures::RULES) {
        ...
        events.extend([
            "round_prestart","round_freeze_end","buytime_ended",
            "round_start","round_end","round_officially_ended","cs_pre_restart",
        ].into_iter().map(String::from));
    }
    ...
    Wanted { player_props, other_props, events }
}
