use crate::feature_extraction::{DecisionMetricsFeatures, TeamDynamicsFeatures};
...
impl TeamDynamicsExtractor {
    ...
    fn extract_team_positioning(
        &self,
        features: &mut TeamDynamicsFeatures,
        team_vectors: &HashMap<u64, Vec<BehavioralVector>>,
    ) {
        ...
        for tick in &common_ticks {
            ...
            let positions: Vec<(f32, f32, f32)> = player_ids
                .iter()
        ...
        if tick_count > 0.0 {
            ...
            if crossfire_opportunities > 0 {
                features.crossfire_setup_effectiveness =
                    effective_crossfires as f32 / crossfire_opportunities as f32;
            }
        }
        ...
        features
            .site_approach_patterns
            .insert("A_site".to_string(), 0.6);
        features
            .site_approach_patterns
            .insert("B_site".to_string(), 0.4);
        features
            .site_approach_patterns
            .insert("mid_control".to_string(), 0.3);
    ...
    fn extract_utility_usage(
        ...
        if coordination_samples > 0 {
            features.support_utility_coordination =
                total_coordination_score / coordination_samples as f32;
        }
    ...
    fn extract_tactical_execution(
        ...
        features
            .execute_success_rate_by_type
            .insert("rush_execute".to_string(), 0.65);
        features
            .execute_success_rate_by_type
            .insert("slow_execute".to_string(), 0.75);
        features
            .execute_success_rate_by_type
            .insert("fake_execute".to_string(), 0.55);
    }
    ...
    fn find_common_ticks(&self, team_vectors: &HashMap<u64, Vec<BehavioralVector>>) -> Vec<u32> {
        ...
        // Get all tick sets
        let tick_sets: Vec<std::collections::HashSet<u32>> = team_vectors
            .values()
    ...
    fn analyze_rotation_timing(&self, _team_vectors: &HashMap<u64, Vec<BehavioralVector>>) -> f32 {
    ...
    fn analyze_utility_coordination(
        &self,
        player_id: u64,
        ...
        for window in vectors.windows(5) {
            ...
            for (&other_player_id, other_vectors) in team_vectors {
                ...
                if let Some(other_vector) = other_vectors
                    .iter()
                    .find(|v| (v.tick as i32 - tick as i32).abs() <= 3)
                {
                    let player_movement =
                        (window[4].vel_x.powi(2) + window[4].vel_y.powi(2)).sqrt();
                    let other_movement =
                        (other_vector.vel_x.powi(2) + other_vector.vel_y.powi(2)).sqrt();
            ...
            if synchronized_movements > 0 {
                coordination_score +=
                    synchronized_movements as f32 / (team_vectors.len() - 1) as f32;
                sample_count += 1;
            }
        }
            ...
            for vectors in team_vectors.values() {
                ...
                if let (Some(start), Some(end)) = (start_pos, end_pos) {
                    let distance_moved = ((end.pos_x - start.pos_x).powi(2)
                        + (end.pos_y - start.pos_y).powi(2))
                    .sqrt();

                    if distance_moved > 300.0 {
    ...
    fn analyze_role_adherence(&self, _team_vectors: &HashMap<u64, Vec<BehavioralVector>>) -> f32 {
    ...
    fn analyze_trade_efficiency(&self, team_vectors: &HashMap<u64, Vec<BehavioralVector>>) -> f32 {
        ...
        for tick in common_ticks.iter().step_by(10) {
            // Sample every 10 ticks
            let positions: Vec<(u64, f32, f32, f32)> = team_vectors
                .iter()
    ...
    fn detect_mid_round_adaptations(
        &self,
        team_vectors: &HashMap<u64, Vec<BehavioralVector>>,
    ) -> f32 {
        ...
        for vectors in team_vectors.values() {
            ...
            for i in 0..segments.len() - 1 {
                ...
                let position_change = ((avg_pos_2.0 - avg_pos_1.0).powi(2)
                    + (avg_pos_2.1 - avg_pos_1.1).powi(2))
                .sqrt();

                if position_change > 500.0 {
    ...
    fn identify_default_strategies(
        &self,
        team_vectors: &HashMap<u64, Vec<BehavioralVector>>,
    ) -> HashMap<String, f32> {
        ...
        let _early_round_positions = self.analyze_early_round_positioning(team_vectors);
    ...
    fn analyze_early_round_positioning(
        &self,
        team_vectors: &HashMap<u64, Vec<BehavioralVector>>,
    ) -> Vec<(f32, f32)> {
...
impl DecisionMetricsExtractor {
    ...
    fn extract_economy_decisions(
        &self,
        features: &mut DecisionMetricsFeatures,
        vectors: &[BehavioralVector],
    ) {
        ...
        if weapon_usage_count > 0 {
            features.buy_efficiency_value_per_dollar =
                total_value_efficiency / weapon_usage_count as f32;
        }
    ...
    fn extract_timing_decisions(
        ...
        for window in vectors.windows(3) {
            ...
            let aim_change_1 =
                ((curr.yaw - prev.yaw).powi(2) + (curr.pitch - prev.pitch).powi(2)).sqrt();
            let aim_change_2 =
                ((next.yaw - curr.yaw).powi(2) + (next.pitch - curr.pitch).powi(2)).sqrt();

            if aim_change_1 > self.reaction_time_threshold
                || aim_change_2 > self.reaction_time_threshold
            {
                total_decisions += 1;
            ...
            let pos_change =
                ((curr.pos_x - prev.pos_x).powi(2) + (curr.pos_y - prev.pos_y).powi(2)).sqrt();
            if pos_change > 200.0 {
                // Significant movement
        ...
        if total_decisions > 0 {
            features.decision_speed_after_first_contact =
                rapid_decisions as f32 / total_decisions as f32;
        }

        // Analyze rotation timing relative to team
        if !rotation_timings.is_empty() {
            let avg_rotation_time =
                rotation_timings.iter().sum::<u32>() as f32 / rotation_timings.len() as f32;
            features.information_based_rotation_timing = 1.0 - (avg_rotation_time / 64.0).min(1.0);
            // Normalize by 1 second
        }
    ...
    fn extract_reaction_metrics(
        ...
        for window in vectors.windows(5) {
            ...
            for i in 1..window.len() {
                ...
                let reaction_magnitude = ((current.yaw - baseline.yaw).powi(2)
                    + (current.pitch - baseline.pitch).powi(2))
                .sqrt();
            ...
            if window.len() >= 3 {
                ...
                let adjustment_needed = ((mid.yaw - baseline.yaw).powi(2)
                    + (mid.pitch - baseline.pitch).powi(2))
                .sqrt();
        ...
        if !adjustment_times.is_empty() {
            let avg_adjustment_time =
                adjustment_times.iter().sum::<f32>() / adjustment_times.len() as f32;
    ...
    fn calculate_weapon_efficiency(&self, vector: &BehavioralVector, _weapon_value: f32) -> f32 {
    ...
    fn analyze_re_aggression_patterns(&self, vectors: &[BehavioralVector]) -> f32 {
        ...
        for (i, vector) in vectors.iter().enumerate() {
            ...
            if previous_positions.len() > 50 {
                // Keep last 50 positions
                previous_positions.remove(0);
            }

            if i > 20 {
                // After some initial movement
                position_samples += 1;

                // Check if current position is similar to a much earlier position
                for &(old_x, old_y) in
                    &previous_positions[..previous_positions.len().saturating_sub(20)]
                {
                    let distance =
                        ((current_pos.0 - old_x).powi(2) + (current_pos.1 - old_y).powi(2)).sqrt();
                    if distance < 100.0 {
                        // Returned to similar position
    ...
    fn calculate_reaction_smoothness(&self, reaction_sequence: &[BehavioralVector]) -> f32 {
        ...
        for window in reaction_sequence.windows(3) {
            let prev_change = ((window[1].yaw - window[0].yaw).powi(2)
                + (window[1].pitch - window[0].pitch).powi(2))
            .sqrt();
            let curr_change = ((window[2].yaw - window[1].yaw).powi(2)
                + (window[2].pitch - window[1].pitch).powi(2))
            .sqrt();
...
mod tests {
    ...
    fn test_team_spread_calculation() {
        ...
        let spread_positions = vec![(0.0, 0.0, 64.0), (1000.0, 0.0, 64.0), (0.0, 1000.0, 64.0)];

        // Test stacked positions
        let stacked_positions = vec![(0.0, 0.0, 64.0), (10.0, 10.0, 64.0), (20.0, 20.0, 64.0)];
