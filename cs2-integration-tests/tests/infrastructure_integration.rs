...
use cs2_integration_tests::{TestInfrastructure, TestDataFactory};
use anyhow::Result;
...
async fn test_complete_infrastructure_setup() -> Result<()> {
    ...
    let minio_health = reqwest::get(&format!("{}/minio/health/live", connections.minio_url)).await?;
...
async fn test_vector_database_operations() -> Result<()> {
    ...
    let response = client
        .put(&format!("{}/collections/{}", connections.qdrant_url, collection_name))
    ...
    let response = client
        .put(&format!("{}/collections/{}/points", connections.qdrant_url, collection_name))
        .json(&point)
        .send()
        .await?;
...
async fn test_ml_pipeline_integration() -> Result<()> {
    ...
    let test_vectors = (0..10).map(|i| {
        TestDataFactory::sample_behavioral_vector(76561198034202275, 1000 + i)
    }).collect::<Vec<_>>();
